        ...
        <div class="couponCon" v-for="(coupon,index) in couponList" v-show="couponList.length!=0">
            <div class="couList">
                ...
                <div class="couList-text">
                    ...
                    <p style="margin:0;font-size: .6rem;color: #999999">有效期：{{coupon.startDate |
                        date('YYYY-MM-DD')}}～{{coupon.endDate | date('YYYY-MM-DD')}}</p>
                    <div style="height: auto">
                        <div @click="explainShow(index+1)" class="couponExplain">礼券说明
                            <div v-if="explainList == index+1" class="arrow_down">></div>
                            <div v-if="explainList != index+1" class="couponRight">></div>
                        </div>
